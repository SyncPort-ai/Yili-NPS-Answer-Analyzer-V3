# coding:utf-8
"""
V1算法的完整副本 - 用于V2系统内部调用
保持与原始opening_question_analysis.py完全一致的逻辑
重命名为v1_legacy_analysis.py避免混淆
"""
...
# 导入V2本地副本，避免与原始V1文件冲突
from .v1_legacy_llm import AzureEmbedding, AzureChat, AzureChatApp
from .v1_legacy_prompts import prompt_create, create_prompt_title
from .v1_legacy_cluster import text_cluster
...
async def v1_legacy_auto_analysis(request_data):
    """
    V1算法的完整副本 - 适配新的输入格式
    
    Args:
        request_data: V1格式的请求数据 {'emotionalType': int, 'taskType': int, 'questionId': str, 'wordInfos': [...]}
    ...
    try:
        # 从request_data提取V1参数
        emotional_type = request_data.get('emotionalType', 1)
        task_type = request_data.get('taskType', 1) 
        question_id = request_data.get('questionId', 'nps_survey')
        word_infos = request_data.get('wordInfos', [])
        ...
        for info in word_infos:
            ...
            word = info.get('word', '')
            for id_val in ids:
                data_list.append({'mark': id_val, 'origion': word})
        
        if not data_list:
            return {'error': '无有效输入数据'}
            
        data = pd.DataFrame(data_list)
        
        # 设置V1分析参数（硬编码，保持与原始逻辑一致）
        question = "产品体验问题"
        theme = "产品体验"
        emotion = "喜欢" if emotional_type == 1 else "不喜欢" if emotional_type == 2 else "其他"
        mode = 'prod'  # 默认使用生产模式
        
        # 调用V1核心分析逻辑
        result = await auto_analysis(question, theme, emotion, data, mode)
        
        # 转换为V1期望的返回格式
        v1_result = {
            'status': 'success',
            'emotionalType': emotional_type,
            'taskType': task_type,
            'questionId': question_id,
            'analysis_result': {
                'total_items': len(result),
                'valid_items': len(result[result['topic'] != '无意义']),
                'topics': result.groupby('topic').size().to_dict(),
                'detailed_results': result.to_dict('records')
            },
            'processing_metadata': {
                'algorithm_version': 'v1_legacy',
                'processing_time': time.time(),
                'data_quality': {
                    'total_input': len(word_infos),
                    'processed_items': len(result),
                    'valid_ratio': len(result[result['topic'] != '无意义']) / len(result) if len(result) > 0 else 0
                }
            }
        }
        ...
        return {
            ...
            'processing_metadata': {
                'algorithm_version': 'v1_legacy',
                'processing_time': time.time(),
                'failed_stage': 'v1_legacy_analysis'
            }
        }
