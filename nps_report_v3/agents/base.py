...
class ConsultingAgent(BaseAgent):
    ...
    async def process_with_confidence_constraints(self, state: Dict[str, Any]) -> AgentResult:
        """Run full processing and annotate results for low confidence."""
        baseline_result = await super().execute(state)
        return self._apply_low_confidence_annotations(baseline_result)
...
class ConfidenceConstrainedAgent(ConsultingAgent):
    ...
    async def execute(self, state: Dict[str, Any]) -> AgentResult:
        ...
        if confidence_level == "low":
            logger.info(f"Agent {self.agent_id} operating under low confidence; annotating output")
            result = self._apply_low_confidence_annotations(result)

        return result

    def _apply_low_confidence_annotations(self, result: AgentResult) -> AgentResult:
        """Tag results with low-confidence warnings without disabling recommendations."""
        if result.warnings is None:
            result.warnings = []
        result.warnings.append("Underlying data confidence is low; interpret these recommendations with caution.")
