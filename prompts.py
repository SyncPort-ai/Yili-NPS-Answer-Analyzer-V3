...
aspect_dict={
    '概念设计': '原料天然、奶源优质、目标群体、乳糖不耐、素食主义、健身、颗粒、蛋白、脂肪、0蔗糖、植物蛋白、植物、水果、口味口感、卡路里、益生菌、维生素、有机、功效性、独特性、接受度、高端、高品质、时尚潮流、天然无添加、年轻活力、安全信赖、营养健康、专业先进、文案宣传、品牌关联、价格、容量、爱国、奥运、低碳环保、包装美观',
...
# '包装设计_不喜欢':f"""示例输入：
# 回答：字太多，很烦跟益生菌挂钩，包装视觉冲击力弱，画面老土，全是文字，抓不住购买欲望
...
def prompt_create(theme, emotion, answers):
    ...
    prompt = f"""你是伊利乳制品公司的分析师，请对以下{theme_specific}多个回答进行观点提取与打标。

要求：
1. 每个回答作为一个整体进行分析，不要将单个回答拆分成更小的单位。
2. 观点提炼：
    - {emotion_specific}
    - 语言要精简明确，但保留关键信息以确保意思完整。
    - 提炼核心主谓结构，可保留必要的宾语或关键修饰词。
    - 去除标点、程度词、非必要修饰词。
    - 确保提炼后的观点能独立表达完整含义。
3. 标签：即观点的评价维度，{'可以从<预设标签>中选择，也可' if theme != "未知设计" else ''}自主提炼，无法提取设为未知。
4. 如果一个回答包含多个观点，请分别列出所有相关的标签和观点对。
5. 如果某个回答无法明确提取出观点或标签，则输出"（未知，未知）"。
